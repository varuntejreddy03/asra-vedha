...
export default function SearchOverlay({ isOpen, onClose, onSelectProduct, catalogProducts = products }: SearchOverlayProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col pt-24 px-6 md:px-12 bg-[#1A0F00]/95 backdrop-blur-md">
      {/* Header element */}
      <div className="max-w-3xl mx-auto w-full flex items-center justify-between border-b border-[#C4A042]/30 pb-4">
        <div className="flex items-center gap-3 flex-grow">
          <Search className="w-5 h-5 text-[#C4A042]" />
          <input
            type="text"
            autoFocus
            value={query}
            ...
            className="bg-transparent text-xl font-display text-[#F5E4B0] focus:outline-none w-full placeholder-[#9a8f80]/60"
          />
        </div>
        
        <button
          onClick={onClose}
          className="p-2 text-[#9a8f80] hover:text-[#C4A042] cursor-pointer transition-colors"
            ...
            <div className="flex flex-wrap gap-2.5">
              {hotSearches.map((term) => (
                <button
                  ...
                  className="bg-[#2D1A00] hover:bg-[#C4A042]/10 text-xs text-[#d1c5b4] hover:text-[#C4A042] px-4 py-2 rounded-full border border-[#C4A042]/30 transition-all cursor-pointer"
                >
                  {term}
                </button>
              ))}
              ...
              <div className="flex flex-col gap-3">
                {results.map((product) => (
                  <div
                    ...
                    className="flex justify-between items-center bg-[#3D2600] border border-[#C4A042]/20 hover:border-[#C4A042]/40 p-3 rounded-lg cursor-pointer group transition-all"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="text-xs font-bold text-[#F5E4B0] group-hover:text-[#C4A042] transition-colors">{product.name}</h4>
                        <p className="text-[10px] text-[#9a8f80] line-clamp-1 mt-0.5">{product.subheading}</p>
                      </div>
                    </div>
                    
                    <div className="flex items-center gap-2 text-[10px] text-[#C4A042]">
                      <span>View</span>
